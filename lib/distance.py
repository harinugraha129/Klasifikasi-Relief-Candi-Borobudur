...
def euclidean_D(x,y,n_feature):
    distance = 0.0
    for i in range (0, n_feature):
        distance += (x[i]-y[i])**2
    return math.sqrt(distance)

def manhattan_D(x,y,n_feature):
    distance = 0.0
    for i in range (0, n_feature):
        distance += math.fabs(x[i]-y[i])
    return distance

def minkowski_D(x,y,n_feature):
    distance = 0.0
    for i in range (0, n_feature):
        distance += (math.fabs(x[i]-y[i]))**3
    return (distance)**1/3

def canberra_D(x,y,n_feature):
    distance = 0.0
    distance2 = 0.0
    for i in range (0, n_feature):
        distance += (math.fabs(x[i]-y[i]))
...
def chebishev_D(x,y,n_feature):
    distance = []
    for i in range (0, n_feature):
        distance.append(math.fabs(x[i]-y[i]))  
    return max(distance)

def cosine_D(x,y,n_feature):
    distance = 0.0
    distance2 = 0.0
    distance3 = 0.0
    for i in range (0, n_feature):
